      ...
      {ropes && (
        <group>
          <mesh position={[0, length / 2.3, 0]} rotation={[Math.PI / 2, 0, 0]} castShadow receiveShadow>
            ...
            <meshStandardMaterial attach="material" color={"#ff8080"} roughness={1} />
          </mesh>
          ...
          <mesh position={[0, -length / 2.3, 0]} rotation={[Math.PI / 2, 0, 0]} castShadow receiveShadow>
            <torusBufferGeometry attach="geometry" args={[OD + 0.025, 0.025, 32, 32]} />
            <meshStandardMaterial attach="material" color={"#ff8080"} roughness={1} /> {/*map={ropeTexture*/}
          </mesh>
        </group>
      )}
    </group>
...
const Scene = () => {
  ...
  return (
    <Canvas shadows gl={{alpha: false}} camera={{position: [0, 10, 0], fov: 80}} flat mode="concurrent">
      ...
      <Suspense fallback={null}>
        ...
        <Environment files={"desert.hdr"} path={"/"} />
